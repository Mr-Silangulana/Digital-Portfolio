# Digital-Portfolio
<P>
My name is Sophumelela Silangulana, and I am currently a 3rd year student at Cape Peninsula University of Technology pursuing a field in ICT (Diploma in Communication Networks), I am also doing a Work Intergrated Learning programme with MICT SETA who placed me with National Library of South Africa. With a strong foundation in networking protocols along with security tools and Application development, and also working on projects such as Network Management, network security and updating network infrastructure. These experiences have allowed me to develop skills in Networking and Security.
 </P>

## Business Communication

### Evidence
- Email example from network project "enhancing network infrasture for Manzomthombo Secondary School (as part of the project you managed earlier in 2025).

  ![](https://github.com/Mr-Silangulana/Digital-Portfolio/blob/main/Emails.png)

###  Reflection
- **Situation**: While working on a project to enhance network infrastructure one the schools in Cape Town, I was responsible for coordinating communication and ensuring technical alignment on the network design.
- **Task**: I needed to communicate complex technical requirements like ACL configurations and routing protocol decisions in a clear and professional manner.  
- **Action**: I used structured written communication (emails, project notes, and presentations) to share updates, summarize discussions and clarify decisions. I applied formal language, technical precision, and visual aids for clarity.  
- **Result**: The team was able to implement consistent configurations, reduce miscommunication, and meet the project deadline. I developed stronger professional communication habits and confidence in presenting technical ideas clearly.
---
## Interview Skills

### Evidence
- Participation in work-readiness and interview preparation sessions.

- Practice interview questions with focus on ICT-related roles (network engineer, systems analyst, IT Support etc.).

  (https://github.com/Mr-Silangulana/Digital-Portfolio/blob/main/Interview%20Questions.mp4)

### Reflection 
- **Situation**: Preparing for a simulated interview as part of my work readiness training.  
- **Task**: To demonstrate effective communication, confidence, and ICT knowledge relevant to industry interviews.  
- **Action**: I researched common ICT interview questions, practiced structuring answers and worked on improving non-verbal communication such as posture and tone. 
- **Result**: My interview responses became more concise and impactful. I gained confidence in articulating technical knowledge and professional goals clearly during formal interviews.
---
## Mock Interview

### Evidence
![](X)


### Reflection 
- **Situation**: During the mock interview, I was evaluated on my ability to respond to real-world interview questions. 
- **Task**: To simulate a professional interview setting and receive feedback on my communication and presentation. 
- **Action**: I applied the STAR method in my answers, maintained confident body language, and focused on industry-relevant examples from my networking coursework. 
- **Result**: I identified areas to reduce such as pacing and improving emphasizing results in my answers, I gained valuable insight into how to perform better in actual job interviews.
---
## Professional Networking

### Evidence

-Created and updated LinkedIn profile showcasing my ICT skills and coursework.
-Participated in online discussions about cloud computing, OpenStack, and 5G networks.
(https://github.com/Mr-Silangulana/Digital-Portfolio/blob/main/Professional%20Networking.mp4)

### Reflection – STAR Technique
- **Situation**: I wanted to build professional connections within the ICT field to explore opportunities and learn from industry professionals. 
- **Task**: My goal was to create an online presence that highlights my technical growth and professionalism.  
- **Action**: I built my LinkedIn profile, shared insights from my CCNA studies and OpenStack project, and connected with peers and mentors in ICT.  
- **Result**: I received valuable feedback, increased visibility in my professional network, and developed a stronger understanding of workplace expectations and opportunities.

---

## Workplace Etiquette X

### Evidence
![](https://github.com/Mr-Silangulana/Digital-Portfolio/blob/main/Tasks%20done%20at%20work.png)

### Reflection
- **Situation**: As I am an Intern with NLSA, I have to maintain professionalism and adhere to ethical workplace standards. Lots of tickets are logged by stuff memmbers for assistance by ICT department. 
- **Task**: I must ensure that every staff member is able to access and use all ICT resources they need, I must make sure they are able to make calls, communicate via teams and also they are able to access the applications used by the Department to ensure that the community gets what they need. Mostly, I make sure that each and every is replied to and resolved.
- **Action**: I ensure that each and every staff member has their system up-to-date with all applications updated to the latest feature. I make sure that every cable in the building is set and installed correctly. I also ensure that all the staff's PCs are protected and are always connected to the Organization's network not from outside networks.  
- **Result**: I am now able to manage staff accounts, change accoutn password as an administrator and also allocating resources to staff members as per SLA and what they need to complete their given tasks.

---

## Repository Info

**Created by:** Sophumelela Silangulana 

**Cell no:** 065 626 0271

**Email:** silangulanas@gmail.com

**LinkedIn:** https://www.linkedin.com/in/sophumelela-silangulana-086799234/
